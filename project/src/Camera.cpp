...
void Camera::mouseInput( double xposIn, double yposIn, float dtime)
{
    ...
    static const float sensitivity = 0.2f * dtime;
...
void Camera::keyboardMovment(float dtime){

    if (glfwGetKey(pWindow, GLFW_KEY_ESCAPE) == GLFW_PRESS)
        glfwSetWindowShouldClose(pWindow, true);

    float moveSpeed = 40.0f * dtime; // Geschwindigkeit der Bewegung anpassen
...
void Camera::update(float dtime)
{
    ...
    keyboardMovment(dtime);
    mouseInput(xpos, ypos, dtime);
